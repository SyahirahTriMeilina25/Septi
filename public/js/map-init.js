        ...
        onEachFeature: function (feature, layer) {
            ...
            layer.on({
                ...
                click: function (e) {
                    ...
                    // Reset previous active layer if exists
                    if (window.activeLayer && window.activeLayer !== layer) {
...
function createJobStatusLayer(data, jobStatus) {
    return L.geoJSON(null, {
        pane: 'polygons',
        style: function (feature) {
            // Check if we're dealing with a province or city
            const isProvince = feature.properties.NAME_2 === undefined;
            const locationName = isProvince ? feature.properties.NAME_1 : feature.properties.NAME_2;

            const count = calculateAlumniCountByStatus(data, locationName, jobStatus);
            return {
                fillColor: getColor(count),
                weight: 1,
                opacity: 1,
        ...
        onEachFeature: function (feature, layer) {
            ...
            // Create appropriate popup based on whether it's a province or city
            const popupContent = isProvince ?
                '<strong>' + provinceName + '</strong><br>' +
                `Jumlah Alumni ${jobStatus}: ` + count :
                '<strong>' + locationName + ', ' + provinceName + '</strong><br>' +
                `Jumlah Alumni ${jobStatus}: ` + count;
...
function calculateAlumniCountByStatus(data, city, status) {
    // Check if city is a province name (for province layers)
    const isProvince = data.some(a => a.province === city);

    if (isProvince) {
        // If city is actually a province name, filter by province instead
        return data.filter(a => a.province === city && a.job_status === status).length;
    } else {
...
L.Control.LayerControl = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        // Create dropdown button with map icon
        ...
        title.innerHTML = '<strong>Layer Peta</strong>';
        ...
        layers.forEach(layer => {
            ...
            L.DomEvent.on(radio, 'change', function (e) {
                ...
                if (e.target.checked) {
                    ...
                    } else if (layer.layer === 'provinceLayer') {
                        // Add province layer
                        if (window.provinceLayer) {
                            map.addLayer(window.provinceLayer);
                        }
                    } else if (layer.layer === 'workingLayer') {
                        // Add both city and province working layers
                        if (window.workingLayerCity) {
                            map.addLayer(window.workingLayerCity);
                        }
                        if (window.workingLayerProvince) {
                            map.addLayer(window.workingLayerProvince);
                        }
                        // Filter for working alumni
                        $('#jobStatusSelect').val('Bekerja').trigger('change');
                    } else if (layer.layer === 'notWorkingLayer') {
                        // Add both city and province not working layers
                        if (window.notWorkingLayerCity) {
                            map.addLayer(window.notWorkingLayerCity);
                        }
                        if (window.notWorkingLayerProvince) {
                            map.addLayer(window.notWorkingLayerProvince);
                        }
                        // Filter for non-working alumni
                        $('#jobStatusSelect').val('Tidak Bekerja').trigger('change');
                    }
                }
            });
...
// Custom control untuk toggle  
L.Control.MarkerToggle = L.Control.extend({
    options: {
        position: 'bottomright' // Changed from bottomleft to bottomright
    },
    ...
    onAdd: function (map) {
        ...
        container.style.display = 'flex';
        container.style.alignItems = 'center';
        container.style.marginBottom = '20px'; // Add some margin to avoid overlapping with legend
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Create panes with proper z-indexing
    map.createPane('polygons');
    map.getPane('polygons').style.zIndex = 400;
    map.getPane('polygons').style.pointerEvents = 'auto'; // Make clickable

    // Create a separate pane for active polygons
    map.createPane('activePolygons');
    map.getPane('activePolygons').style.zIndex = 450;
    map.getPane('activePolygons').style.pointerEvents = 'auto'; // Make clickable
    ...
    function updateMap(filteredData) {
        ...
        // Remove all existing legends first
        if (window.legend) {
            map.removeControl(window.legend);
            window.legend = null;
        }

        // Check for and remove any other legends that might be present
        ...
        // Reset active layer when updating map
        window.activeLayer = null;
        ...
        // Always create and add a new legend when updating map
    ...
    $.getJSON('/geojson/kota.geojson', function (kotaData) {
        ...
        // Create working/not working layers for cities only
        window.workingLayerCity = createJobStatusLayer(alumniData, 'Bekerja');
        window.notWorkingLayerCity = createJobStatusLayer(alumniData, 'Tidak Bekerja');

        window.workingLayerCity.addData(kotaData);
        window.notWorkingLayerCity.addData(kotaData);

        // For backward compatibility - still use the old references but with the city versions
        window.workingLayer = window.workingLayerCity;
        window.notWorkingLayer = window.notWorkingLayerCity;
    ...
    $.getJSON('/geojson/provinsi.geojson', function (provinsiData) {
        ...
        // Create working/not working layers for provinces
        window.workingLayerProvince = createJobStatusLayer(alumniData, 'Bekerja');
        window.notWorkingLayerProvince = createJobStatusLayer(alumniData, 'Tidak Bekerja');

        window.workingLayerProvince.addData(provinsiData);
        window.notWorkingLayerProvince.addData(provinsiData);
    });

    // Initialize the application
    initializeData();

    // Remove any duplicate legends that might have been created during initialization
    document.querySelectorAll('.info.legend').forEach(function (legendElement, index) {
        // Keep only the first legend, remove any others
        if (index > 0 && legendElement && legendElement.parentNode) {
            legendElement.parentNode.removeChild(legendElement);
        }
    });
