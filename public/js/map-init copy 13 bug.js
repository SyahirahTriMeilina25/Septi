...
L.Control.LayerControl = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        const layers = [
            { name: 'Sebaran per Kota', id: 'cityLayer' },
            { name: 'Sebaran per Provinsi', id: 'provinceLayer' },
            { name: 'Alumni Bekerja', id: 'workingLayer' },
            { name: 'Alumni Tidak Bekerja', id: 'notWorkingLayer' }
        ];
        ...
        layers.forEach(layer => {
            ...
            L.DomEvent.on(radio, 'change', function () {
                console.log(`Switching to layer: ${layer.id}`);

                // Remove all existing layers
                if (window.cityLayer) map.removeLayer(window.cityLayer);
                if (window.provinceLayer) map.removeLayer(window.provinceLayer);
                if (window.workingLayer) map.removeLayer(window.workingLayer);
                if (window.notWorkingLayer) map.removeLayer(window.notWorkingLayer);
                ...
                if (window[layer.id]) {
                    ...
                    map.addLayer(window[layer.id]);

                    // Reset active layer
                    window.activeLayer = null;

                    // Update job status filter based on selected layer
                    const jobStatusSelect = $('#jobStatusSelect');
                    switch (layer.id) {
                        case 'workingLayer':
                            jobStatusSelect.val('Bekerja').trigger('change');
                            break;
                        case 'notWorkingLayer':
                            jobStatusSelect.val('Tidak Bekerja').trigger('change');
                            break;
                        case 'provinceLayer':
                            // When switching to province layer, clear any city selection
                            // but keep any province selection
                            $('#citySelect').val('').trigger('change');
                            break;
                        default:
                            // Reset job status filter for other layers
                            jobStatusSelect.val('').trigger('change');
                    }
                } else {
                    console.error(`Layer ${layer.id} not found in window object`);
                }
            });

            // Set cityLayer as default
            if (layer.id === 'cityLayer') {
                radio.checked = true;
            }
        });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function filterAlumni() {
        ...
        const activeLayer = document.querySelector('input[name="layerControl"]:checked').value;
        ...
        const filteredData = alumniData.filter(alumni => {
            ...
            // Additional layer-specific filtering
            let matchLayer = true;
            if (activeLayer === 'workingLayer') {
                matchLayer = alumni.job_status === 'Bekerja';
            } else if (activeLayer === 'notWorkingLayer') {
                matchLayer = alumni.job_status === 'Tidak Bekerja';
            }

            return matchProvince && matchCity && matchName && matchCompany &&
                matchYear && matchJob && matchJobStatus && matchSalary && matchLayer;
        });
    ...
    function initializeData() {
        try {
            ...
            // We'll load markers only once with the initial update
    ...
    function resetFilters() {
        try {
            ...
            if (window.cityLayer) {
                ...
                // Update the layer control radio buttons
                const cityLayerRadio = document.getElementById('cityLayer');
                if (cityLayerRadio) cityLayerRadio.checked = true;
            }
